...
export default function PreviewCard({ form }) {
  return (
    <div className="sticky top-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60">
      {/* Banner */}
      <div className="relative h-44" style={{ background: form.cover_color }}>
        {form.banner && (
      ...
      <div className="relative px-6 pb-6">
        ...
        <h2 className="mt-5 text-2xl font-bold text-slate-900">
          {form.name || 'Nombre del emprendimiento'}
        </h2>
        ...
        <div className="mt-8 space-y-3">
          ...
          {form.phone && <Item icon={<Phone size={17} />} text={form.phone} />}

          {form.website && <Item icon={<Globe size={17} />} text={form.website} />}

          {(form.city || form.region || form.country) && (
            <Item
              icon={<MapPin size={17} />}
              text={[form.city, form.region, form.country].filter(Boolean).join(', ')}
            />
          )}
        </div>
